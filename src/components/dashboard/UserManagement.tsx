...
const TableSortHeader: React.FC<{
  label: string;
  sortKey: keyof UserDisplay;
  sortConfig: SortConfig;
  requestSort: (key: keyof UserDisplay) => void;
...
const UserManagement: React.FC = () => {
  ...
  const fetchUsers = useCallback(async () => {
    ...
    try {
      const [verifiedUsers, unverifiedUsers] = await Promise.all([
        getUsers(token, searchTerm, undefined, true),
        getUsers(token, searchTerm, undefined, false),
      ]);
      const combinedMap = new Map<string, GetUsersResponse>();
      verifiedUsers?.forEach((u) => combinedMap.set(u.id, u));
      unverifiedUsers?.forEach((u) => { if (!combinedMap.has(u.id)) combinedMap.set(u.id, u); });
      const mapped: UserDisplay[] = Array.from(combinedMap.values()).map((u) => ({
        id: u.id,
        name: u.name,
        phone: u.phone,
        isPJT: !!u.region_id && u.region_id !== "00000000-0000-0000-0000-000000000000",
        regionName: u.desa_kelurahan || "",
      }));
      setUsersList(mapped);
    } catch (error: any) {
      setUsersList([]);
  ...
  const handleSuccess = () => fetchUsers();
                      ...
                      <td className="py-3 px-4">
                        <span
                          className={`inline-flex items-center px-3 py-1 text-xs font-semibold rounded-full ${
                            u.isPJT ? "bg-primary/20 text-primary" : "bg-red-100 text-red-700"
                          }`}
                        >
